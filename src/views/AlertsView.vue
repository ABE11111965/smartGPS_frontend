...
const plates = computed(() => [...new Set(alerts.value.map((item) => item.plate))])
...
async function update() {
  ...
  saving.value = true
  try {
    await store.updateAlert(selected.value.id, actionKind.value, actionForm.resolution, actionForm.remark)
    actionDialog.value = false
    ElMessage.success(actionKind.value === 'ACKNOWLEDGED' ? '告警已确认' : '告警已关闭')
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '告警处理失败')
  } finally {
    saving.value = false
  }
}
  ...
  <div class="view-stack">
    <section class="metric-grid alert-metrics alert-conveyor-scene">
      ...
      <div class="alert-conveyor-track">
        <div class="alert-conveyor-set">
          <article class="metric-card red"><span class="box-label">CRIT</span><span class="box-barcode"></span><div class="metric-icon"><el-icon><WarningFilled /></el-icon></div><div><span>严重告警</span><strong>{{ store.usingDemo ? alerts.filter(a => a.severity === 'CRITICAL').length : stats.critical }}</strong><small>需要立即处理</small></div></article>
          <article class="metric-card orange"><span class="box-label">TODO</span><span class="box-barcode"></span><div class="metric-icon"><el-icon><BellFilled /></el-icon></div><div><span>待处理</span><strong>{{ store.usingDemo ? alerts.filter(a => a.status === 'PENDING').length : stats.pending }}</strong><small>较昨日 -3</small></div></article>
          <article class="metric-card blue"><span class="box-label">ACK</span><span class="box-barcode"></span><div class="metric-icon"><el-icon><CircleCheckFilled /></el-icon></div><div><span>已确认</span><strong>{{ alerts.filter(a => a.status === 'ACKNOWLEDGED').length }}</strong><small>处理中</small></div></article>
          <article class="metric-card green"><span class="box-label">DONE</span><span class="box-barcode"></span><div class="metric-icon"><el-icon><Select /></el-icon></div><div><span>今日已关闭</span><strong>{{ store.usingDemo ? alerts.filter(a => a.status === 'RESOLVED').length : stats.resolved }}</strong><small>平均 18 分钟</small></div></article>
        </div>
      </div>
    </section>
    ...
    <section class="alerts-layout">
      <article class="panel alerts-table-panel">
        ...
        <div class="alert-list">
          <button v-for="item in filtered" :key="item.id" :class="{ active: selected?.id === item.id }" @click="selectAlert(item)">
            <span class="severity-icon" :class="[item.severity.toLowerCase(), alertTone(item.type)]"><el-icon><Warning /></el-icon></span>
            <span class="alert-main"><strong>{{ item.title }}</strong><small>{{ item.plate }} · {{ item.location }}</small></span>
            <span class="severity-label" :class="item.severity.toLowerCase()">{{ severityText[item.severity] }}</span>
            <span class="alert-status">{{ statusText[item.status] }}</span>
      ...
      <aside v-if="selected" class="panel alert-detail">
        <div class="alert-detail-head" :class="[selected.severity.toLowerCase(), alertTone(selected.type)]">
          <span class="severity-icon" :class="[selected.severity.toLowerCase(), alertTone(selected.type)]"><el-icon><WarningFilled /></el-icon></span>
          <div><span>{{ severityText[selected.severity] }}告警</span><h3>{{ selected.title }}</h3><small>{{ selected.id }} · {{ selected.createdAt }}</small></div>
        </div>
        ...
        <dl class="info-list">
          ...
          <div><dt>当前状态</dt><dd>{{ statusText[selected.status] }}</dd></div>
        </dl>
        <div class="mini-location" :class="alertTone(selected.type)"><div class="long-route"></div><span><el-icon><LocationFilled /></el-icon></span></div>
        ...
        <el-button v-if="selected.status !== 'RESOLVED'" size="large" @click="openAction('RESOLVED')">关闭告警</el-button>
      </aside>
    </section>
