...
function fitMapToVehicles(path: number[][]) {
  ...
  if (!hasFitInitialView) {
    ...
    window.setTimeout(() => {
      ...
      if (typeof zoom === 'number' && zoom < 8.6) map.setZoom(8.8)
...
function renderMapData() {
  ...
  fitMapToVehicles(path)
}

async function initMap() {
  if (!serviceConfig.amapKey || !mapEl.value) return
  try {
...
watch(() => [props.vehicles, props.selectedPlate], () => nextTick(renderMapData), { deep: true })
...
<template>
  <div class="smart-map" :style="{ height }">
    ...
    <div v-if="usingRealMap" class="map-marker-overlay">
      <button
        v-for="point in fallbackPoints"
        :key="`overlay-${point.vehicle.plate}`"
        class="map-truck"
